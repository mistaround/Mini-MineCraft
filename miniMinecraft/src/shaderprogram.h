...
public:
    ...
    int unifTexture2D; // A handle to the "uniform" sampler2D that will be used to read the texture containing the scene render
    int unifNormalMap; // A handle to the "uniform" sampler2D that will be used to read the normal map for the scene render
    int unifTime; // A handle for the "uniform" float representing time in the shader
    int unifPostType; // 0 Default 1 Water 2 Lava
    int unifPostQuad; // The post effected quad
    int unifDimensions; // The dimensions of the screen
    int unifEye; // The position of the eye
...
public:
    ...
    // Pass the given post effect type
    void setPostType(int type);
    // Pass the dimensions of the screen
    void setDimensions(int w, int h);
    // Pass the Eye position to the shader
    ...
    void drawInterleavedOpaque(InterleavedDrawable &d, int textureSlot = 0, int normalMapSlot = 1);
    // Draw the given object to our screen using interleaved vbo
    void drawInterleavedTransparent(InterleavedDrawable &d, int textureSlot = 0, int normalMapSlot = 1);
    // Draw the post effected image to screen
    void drawPostEffected(Drawable &d);
