    ...
    for(int z = -1; z <= 1; z++)
    {
        for(int y = -1; y <= 1; y++)
        {
            for(int x = -1; x <= 1; x++)
            {
                ...
                // Animate the point
                point = 0.5 + 0.5 * sin(u_Time * TIME_SCALE * 0.01 + 6.2831 * point); // 0 to 1 range
...
//    outColor = vec3(ndc * 0.5 + 0.5, 1);
    ...
    if (time > 0.33) {
        ...
        color = mix(duskColor, daytimeColor, (time + 0.33) / 0.66);
        if (time > 0) {
    ...
    // night sky random stars
//    if (time < -0.33) {
//        float r = random2(rayDir.xy * 1000.0).x;
//        if (r > 0.999) {
//            color = vec3(1);
//        }
//    }
    ...
    // looking into just the sky
    if (time > -0.33 && time < 0.33) {
...
#define DUSK_THRESHOLD -0.1
    ...
    // for smooth transition between day and night
    color = mix(time > 0 ? daytimeColor : duskColor, color, smoothstep(0.f, 1.f, 1 - abs(time) / 0.33));
    ...
    if(angle < sunSize) {
        ...
        // Corona of sun, mix with sky color
            ...
            color = mix(sunColor, color, (angle - 7.5) / 22.5);
        }
    }
